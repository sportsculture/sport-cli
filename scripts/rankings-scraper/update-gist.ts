...
class GistUpdater {
  ...
  async update(data: RankingsData): Promise<void> {
    try {
      ...
      console.log(`   Raw: ${response.data.files?.[this.config.filename]?.raw_url}`);
  ...
  private validateData(data: RankingsData): void {
    ...
    const totalModels = data.snapshots.reduce((sum, s) => sum + s.models.length, 0);
    if (totalModels === 0) {
      throw new Error('No models found in any snapshot');
    }
    
    console.log(`✓ Data validation passed: ${data.snapshots.length} snapshots, ${totalModels} total models`);
  }
  ...
  async compareWithCurrent(newData: RankingsData): Promise<void> {
    try {
      ...
      const hoursDiff = (newTime.getTime() - currentTime.getTime()) / (1000 * 60 * 60);
      
      console.log(`\n📊 Comparison with current data:`);
      console.log(`   Last update: ${currentData.timestamp} (${hoursDiff.toFixed(1)} hours ago)`);
      ...
      const currentOverall = currentData.snapshots.find(s => s.category === 'overall' && s.period === 'day');
      const newOverall = newData.snapshots.find(s => s.category === 'overall' && s.period === 'day');
