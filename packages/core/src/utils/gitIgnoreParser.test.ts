...
`;
    ...
    it('should handle custom patterns file name', () => {
      ...
      vi.mocked(fs.readFileSync).mockImplementation((filePath) => {
        if (filePath === path.join(mockProjectRoot, '.sprtscltrignore')) {
          return 'temp/\n*.tmp';
        }
        throw new Error('ENOENT');
      });

      parser.loadPatterns('.sprtscltrignore');
      expect(parser.getPatterns()).toEqual(['temp/', '*.tmp']);
      expect(parser.isIgnored('temp/file.txt')).toBe(true);
      expect(parser.isIgnored('src/file.tmp')).toBe(true);
    });

    it('should initialize without errors when no .sprtscltrignore exists', () => {
      expect(() => parser.loadPatterns('.sprtscltrignore')).not.toThrow();
    });
  });
