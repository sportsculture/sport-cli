...
import { getTopRecommendations, scoreModels, formatModelForDisplay } from '@google/gemini-cli-core';

// Helper function to get dynamic recommendations
async function getDynamicRecommendations(allModels: Array<{model?: any; configured: boolean}>): Promise<Array<{id: string; recommendedFor: string; provider: string}>> {
  ...
  if (openRouterModels.length === 0) {
    ...
    return [
      { id: 'gemini-2.5-pro', recommendedFor: 'Free Advanced Model', provider: 'Gemini' },
      { id: 'gemini-2.5-flash', recommendedFor: 'Free Fast Model', provider: 'Gemini' },
  ...
  const recommendations: Array<{id: string; recommendedFor: string; provider: string}> = [];
  ...
  const bestOpenAI = allScored.find(m => 
  ...
  const bestAnthropic = allScored.find(m => 
    !selectedIds.has(m.id) && (
  ...
  const bestGoogle = allScored.find(m => 
    !selectedIds.has(m.id) && (
  ...
  const bestXAI = allScored.find(m => 
    !selectedIds.has(m.id) && (
      m.id.includes('grok') ||
      m.id.includes('xai/')
    )
  ...
  const bestFree = allScored.find(m => 
    !selectedIds.has(m.id) && 
  ...
  for (const geminiModel of geminiModels) {
    recommendations.push({
      id: geminiModel.id,
      recommendedFor: geminiModel.id.includes('pro') ? 'Free Advanced Model' : 'Free Fast Model',
      provider: 'Gemini'
    });
...
export const modelsCommand: SlashCommand = {
  ...
  action: async (context, args) => {
    ...
    if (!showAll) {
      ...
      for (const rec of recommendations) {
        ...
        if (found && found.model) {
          ...
          if (model.pricing?.inputPer1k && model.pricing?.outputPer1k) {
            const inputCost = (parseFloat(model.pricing.inputPer1k) * 1000).toFixed(2);
            const outputCost = (parseFloat(model.pricing.outputPer1k) * 1000).toFixed(2);
          ...
          const modelName = model.id.length > 24 ? model.id.substring(0, 20) + '...' : model.id;
      ...
      modelsContent += '_💡 \x1b[1mTip\x1b[0m: Type `/models all` to see all available models grouped by provider._\n\n';
    } else {
